...
- 核心模型子系统
  - 新增 `src/core/modeling` 子系统，提供 `ModelFormat`、`ModelImportSummary`、`ModelImportManager`。
  - `.stp/.step` 使用 `STEPControl_Reader` 导入，`.brep` 使用 `BRepTools::Read` 导入。
  - 导入后统一统计根 shape 数量和基础拓扑数量，不解析颜色、名称树、装配层级与 PMI。
- UI 与文件打开流
  - 将当前中心工作区泛化为文档工作区，保留脚本标签能力，并新增只读 `ModelSummaryWidget` 标签类型。
...
- `core::ModelFormat`
- `core::ModelImportSummary`
- `core::ModelImportManager::isSupportedFile(path)`
- `core::ModelImportManager::importFile(path)`
- 文档工作区新增统一入口：`openPath(path)`
- `ui::ModelSummaryWidget::setSummary(const core::ModelImportSummary &summary)`
